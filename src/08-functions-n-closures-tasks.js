...
function getPolynom(...args) {
  ...
  return (x) => {
    ...
    if (args.length === 1) return c;
    if (args.length === 2) return (b * x + c);
    return (a * x ** a + b * x + c);
  };
}
...
// function retry(func, attempts) {
//   return func.call(this, attempts);
// }
function retry(func, attempts) {
  return () => {
    for (let i = 0; i < attempts; i += 1) {
      try {
        ...
        if (i === attempts - 1) return attempts;
      }
    }
    return attempts;
  };
...
function logger(func, logFunc) {
  return (...args) => {
    const logStarts = `${func.name}(${JSON.stringify(args).slice(1, -1)}) starts`;

    logFunc(logStarts);

    const result = func(...args);

    const logEnds = `${func.name}(${JSON.stringify(args).slice(1, -1)}) ends`;
