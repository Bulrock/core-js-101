...
function getRectangleString(width, height) {
  ...
  const vertical = '│';
  const space = ' ';
  const topRow = `┌${horizontal.repeat(width - 2)}┐\n`;
  const midleRow = `${vertical}${space.repeat(width - 2)}${vertical}\n`;
  const bottomRow = `└${horizontal.repeat(width - 2)}┘\n`;
  result = topRow + midleRow.repeat(height - 2) + bottomRow;
  return result;
}
  ...
  for (let i = 0; i < str.length; i++) {
    ...
    if (str.charCodeAt(i) < 91 && str.charCodeAt(i) > 64) {
      if ((str.charCodeAt(i) + 13) > 90) {
        result += String.fromCharCode(64 - (90 - (str.charCodeAt(i) + 13)));
      } else {
    ...
    if (str.charCodeAt(i) < 123 && str.charCodeAt(i) > 96) {
      if ((str.charCodeAt(i) + 13) > 122) {
        result += String.fromCharCode(96 - (122 - (str.charCodeAt(i) + 13)));
      } else {
        result += String.fromCharCode(str.charCodeAt(i) + 13);
      }
    }
