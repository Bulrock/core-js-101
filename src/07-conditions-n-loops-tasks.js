...
function isTriangle(a, b, c) {
  const arr = [a, b, c].sort((elem1, elem2) => elem1 - elem2);
...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  ...
  return `${left}${[a, b].sort((elem1, elem2) => elem1 - elem2).join(', ')}${right}`;
}
...
function getDigitalRoot(num) {
  if (num < 9) return num;
  return getDigitalRoot(Number(String(num).split('').reduce((prev, current) => Number(prev) + Number(current), 0)));
}
